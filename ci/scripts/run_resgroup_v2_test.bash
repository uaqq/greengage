...
#
# The cgroup v1 suite (run_resgroup_test.bash) needs a QEMU VM because
# GitHub-hosted runners boot with cgroup v2 only. The v2 suite is the opposite:
# the runner already provides cgroup v2, so a privileged container that shares
# the host cgroup namespace and the host cgroup v2 filesystem is enough. All
# cluster and cgroup setup lives in concourse/scripts/ic_gpdb_resgroup_v2.bash.
#
# gpdemo-datadirs and testtablespace are bind-mounted from a regular host
# filesystem: IO_LIMIT tests need real block devices, and
# ic_gpdb_resgroup_v2.bash rejects Docker overlay / tmpfs paths. Docker creates
# the bind-mount sources if missing; setup_gpadmin_user.bash makes them writable.
#
