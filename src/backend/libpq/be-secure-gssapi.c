...
be_gssapi_write(Port *port, void *ptr, size_t len)
{
	...
	while (bytes_to_encrypt || PqGSSSendLength)
	{
		...
		{
			pg_GSS_error_be(COMMERROR, _("GSSAPI wrap error"), major, minor);
			errno = ECONNRESET;
			return -1;
		}
		if (conf_state == 0)
...
be_gssapi_read(Port *port, void *ptr, size_t len)
{
	...
	while (bytes_returned < len)
	{
		...
		if (major != GSS_S_COMPLETE)
		{
			pg_GSS_error_be(COMMERROR, _("GSSAPI unwrap error"), major, minor);
			errno = ECONNRESET;
			return -1;
		}
		if (conf_state == 0)
...
secure_open_gssapi(Port *port)
{
	...
	while (true)
	{
		...
		if (GSS_ERROR(major))
		{
			pg_GSS_error_be(COMMERROR, _("could not accept GSSAPI security context"),
						 major, minor);
			gss_release_buffer(&minor, &output);
			return -1;
		}
	...
	if (GSS_ERROR(major))
	{
		pg_GSS_error_be(COMMERROR, _("GSSAPI size check error"), major, minor);
		return -1;
	}
