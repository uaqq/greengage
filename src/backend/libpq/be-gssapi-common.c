 ...
 *
 * GPDB: the elevel is chosen by the caller via the "severity" argument,
 * rather than always being COMMERROR as in upstream.  COMMERROR is used
 * when we daren't try to send the error to the client, as that'd likely
 * lead to infinite recursion when elog.c tries to write to the client.
 *
