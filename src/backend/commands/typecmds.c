...
DefineRange(CreateRangeStmt *stmt)
{
	...
	/* Create the multirange's array type (name computed earlier) */
	...
	/*
	 * GPDB: dispatch before freeing multirangeTypeName/multirangeArrayName.
	 * For a user-specified multirange name, multirangeTypeName aliases the
	 * String value inside stmt->params (QualifiedNameGetCreationNamespace does
	 * not copy), so pfree()ing it before CdbDispatchUtilityStatement()
	 * serializes stmt would dispatch a freed (empty) name to the segments.
	 */
	if (Gp_role == GP_ROLE_DISPATCH)
		CdbDispatchUtilityStatement((Node *) stmt,
									DF_CANCEL_ON_ERROR|
