			  ...
			  bool isInternal, bool in_partition)
{
	...
	if (!trigger_exists)
	{
		 ...
		 *
		 * GPDB: For RI constraint triggers, the trigger's name is derived from
		 * the trigger OID. That creates a chicken-and-egg problem with the
		 * usual GPDB OID dispatching mechanism. In a QE, we cannot look up the
		 * trigger OID to use by trigger name, because the trigger name is
		 * derived from the OID. To work around that, we use more fields as the
		 * key. For a user-defined trigger, tgrelid and the trigger name should
		 * be enough. For internal triggers, we use the name prefix together
		 * with constraint OID and function OID. That should be unique: there
		 * should be no need to have more than one internal trigger with the
		 * same function for one constraint.
		 */
