...
FileClose(File file)
{
	...
	if (vfdP->fdstate & FD_DELETE_AT_CLOSE)
	{
		...
		if (unlink(vfdP->fileName))
			/*
			 * GPDB: keep this at DEBUG1 rather than LOG; MPP execution
			 * generates many temporary/spill files and logging every failed
			 * cleanup at LOG level would be excessively noisy.
			 */
			ereport(DEBUG1,
					(errcode_for_file_access(),
		...
		{
			errno = stat_errno;
			/* GPDB: DEBUG1 instead of LOG, see note on the unlink above */
			ereport(DEBUG1,
					(errcode_for_file_access(),
